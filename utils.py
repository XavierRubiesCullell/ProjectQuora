...
def truncate(tokens_a, tokens_b, max_seq_length):
    ...
    len_b = len(tokens_b)
    len_t = len_a + len_b
    if len_a < max_seq_length/2:
        return tokens_a, tokens_b[:int(min(max_seq_length-len_a, len_b))]
    if len_b < max_seq_length/2:
        return tokens_a[:int(min(max_seq_length-len_b, len_a))], tokens_b
    return tokens_a[:int(min(max_seq_length/2, len_a))], tokens_b[:int(min(max_seq_length/2, len_b))]
...
def WriteCSV(questions, targets, file_name):
    '''
    Writes a DataFrame to a CSV
    '''
    result = pd.concat([questions, targets], axis=1)
